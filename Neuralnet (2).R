...
nn.holdout <- read.table("VAL_SAMPLE.csv", sep = ",", header = T)
...
nn.dev$Over18=as.numeric(nn.dev$Over18)
nn.dev$OverTime=as.numeric(nn.dev$OverTime)

#neural net model
library(neuralnet)

nn1 <- neuralnet(formula = Attrition ~ Age +  BusinessTravel+	DailyRate+	Department+	DistanceFromHome+	Education+	EducationField+	EmployeeCount+	EnvironmentSatisfaction+	Gender+	HourlyRate+	JobInvolvement+	JobLevel+	JobRole+	JobSatisfaction+	MaritalStatus+	MonthlyIncome+	MonthlyRate+	NumCompaniesWorked+	Over18+	OverTime+	PercentSalaryHike+	PerformanceRating+	RelationshipSatisfaction+	StandardHours+	StockOptionLevel+	TotalWorkingYears+	TrainingTimesLastYear+	WorkLifeBalance+	YearsAtCompany+	YearsInCurrentRole+	YearsSinceLastPromotion+	YearsWithCurrManager,
                 data = nn.dev, 
                 hidden = 6,
                 err.fct = "sse",
                 linear.output = FALSE,
...
nn.holdout$Over18=as.numeric(nn.holdout$Over18)
...
str(x.scaled2)
x.scaled2=x.scaled2[,-9]
x.scaled2=x.scaled2[,-20]
x.scaled2=x.scaled2[,-24]
...
library(ineq)
gini = ineq(misClassTable$Predict.score, type="Gini")
