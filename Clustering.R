...
RCDF <- read.csv("Soapclasscompa.csv", header=TRUE)
View(RCDF)

?dist  ## to get help on distance function
d.euc <- dist(x=RCDF[,2:4], method = "euclidean") 
d.euc
...
KRCDF <- read.csv("Engg_College_Data.csv", header=TRUE)
## scale function standardizes the values
...
set.seed(1234)
nc <- NbClust(KRCDF[,c(-1,-2)], min.nc=2, max.nc=5, method="kmeans")
table(nc$Best.n[1,])
